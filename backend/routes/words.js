...
router.get("/stats", async (req, res) => {
  try {
    ...
    const knownWords = await Word.countDocuments({
      userId: req.userId,
      status: "known",
    });
    const unknownWords = await Word.countDocuments({
      userId: req.userId,
      status: "unknown",
    });
...
router.post("/", async (req, res) => {
  try {
    const { word, pinyin, definition, translation, examples, sourceImage } =
      req.body;
    ...
    if (existingWord) {
      return res
        .status(400)
        .json({ error: "Word already exists in your list" });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to add word", message: error.message });
  }
});
...
router.post("/batch", async (req, res) => {
  try {
    ...
    for (const wordItem of words) {
      ...
      const wordPinyin =
        typeof wordItem === "object" ? wordItem.pinyin : undefined;
    ...
    res
      .status(500)
      .json({ error: "Failed to add words", message: error.message });
  }
});
...
router.patch("/:wordId/status", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update word", message: error.message });
  }
});
...
router.delete("/:wordId", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete word", message: error.message });
  }
});
...
router.post("/:wordId/generate-details", async (req, res) => {
  try {
    ...
    if (updateType === "compounds") {
      ...
      console.log(
        `✅ ${actionCompleted} ${word.compounds.length} compounds and ${word.examples.length} examples`
      );
    }
    ...
    res.json({
      message: force
        ...
        : "Word details generated successfully",
      word,
    });
  } catch (error) {
